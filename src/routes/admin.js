...
router.post('/api-keys', authenticateAdmin, async (req, res) => {
  try {
    ...
    // 验证keyType字段
    if (keyType !== undefined && !['cc', 'aws', 'databricks', 'anthropic'].includes(keyType)) {
      return res.status(400).json({ error: 'Key type must be one of: cc, aws, databricks, anthropic' });
    }
...
router.put('/api-keys/:keyId', authenticateAdmin, async (req, res) => {
  try {
    const { keyId } = req.params;
    const { tokenLimit, concurrencyLimit, rateLimitWindow, rateLimitRequests, claudeAccountId, geminiAccountId, permissions, enableModelRestriction, restrictedModels, enableClientRestriction, allowedClients, expiresAt, dailyCostLimit, keyType } = req.body;
    ...
    if (keyType !== undefined) {
      if (!['cc', 'aws', 'databricks', 'anthropic'].includes(keyType)) {
        return res.status(400).json({ error: 'Key type must be one of: cc, aws, databricks, anthropic' });
      }
      updates.keyType = keyType;
    }
...
// 获取单个Claude账户详情（包含敏感信息）
router.get('/claude-accounts/:accountId', authenticateAdmin, async (req, res) => {
  try {
    const { accountId } = req.params;
    const account = await claudeAccountService.getAccountWithTokens(accountId);
    ...
    logger.error('❌ Failed to get Claude account details:', error);
    res.status(500).json({ error: 'Failed to get account details', message: error.message });
  }
});
