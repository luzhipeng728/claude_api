...
class ClaudeAccountService {
  ...
  // 📋 获取单个账户详情（包含敏感信息）
  async getAccountWithTokens(accountId) {
    try {
      ...
      const decryptedAccount = {
        ...
        email: account.email ? this._decryptSensitiveData(account.email) : '',
        ...
        // 包含解密的tokens
        accessToken: account.accessToken ? this._decryptSensitiveData(account.accessToken) : '',
        refreshToken: account.refreshToken ? this._decryptSensitiveData(account.refreshToken) : ''
      };
      
      return decryptedAccount;
    } catch (error) {
      logger.error('❌ Failed to get Claude account with tokens:', error);
      throw error;
    }
  }
