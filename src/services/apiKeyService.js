...
class ApiKeyService {
  ...
  async generateApiKey(options = {}) {
    const {
      ...
      allowedClients = [],
      dailyCostLimit = 0,
      keyType = 'cc' // 'cc', 'aws', 'databricks', 'anthropic'
    } = options;
    ...
    const keyData = {
      ...
      keyType: keyType || 'cc', // 'cc', 'aws', 'databricks', 'anthropic'
  ...
  async getAllApiKeys() {
    try {
      ...
      for (const key of apiKeys) {
        ...
        key.keyType = key.keyType || 'cc'; // 兼容旧数据，默认cc类型
  ...
  async updateApiKey(keyId, updates) {
    try {
      ...
      // 允许更新的字段
      const allowedUpdates = ['name', 'description', 'tokenLimit', 'concurrencyLimit', 'rateLimitWindow', 'rateLimitRequests', 'isActive', 'claudeAccountId', 'geminiAccountId', 'permissions', 'expiresAt', 'enableModelRestriction', 'restrictedModels', 'enableClientRestriction', 'allowedClients', 'dailyCostLimit', 'keyType'];
