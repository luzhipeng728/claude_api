...
Claude Relay Service 是一个功能完整的 Claude API 中转服务，支持多账户管理、API Key 认证、代理配置和现代化 Web 管理界面。该服务作为客户端（如 SillyTavern、Claude Code CLI）与 Anthropic API 之间的中间件，提供认证、限流、监控等功能。同时支持 Gemini API 代理，实现多模型统一接入。

## 核心架构

### 关键架构概念
- **代理认证流**: 客户端用自建API Key → 验证 → 获取Claude账户OAuth token → 转发到Anthropic
- **多模型支持**: 同时支持Claude（OAuth）和Gemini（API Key）两种认证方式
...
- **实时成本跟踪**: 基于token使用量的实时成本计算和多维度统计
- **Claude Code集成**: 特殊处理Claude Code CLI请求，确保正确的头部和系统提示词

### 主要服务组件
- **claudeRelayService.js**: 核心代理服务，处理请求转发和流式响应，Claude Code请求检测
- **claudeAccountService.js**: Claude账户管理，OAuth token刷新和账户选择，AES数据加密
- **apiKeyService.js**: API Key管理，验证、限流、使用统计和哈希存储
- **oauthHelper.js**: OAuth工具，PKCE流程实现和代理支持
- **geminiRelayService.js**: Gemini API代理服务，支持多模型和流式响应
- **geminiAccountService.js**: Gemini账户管理，API Key验证和轮换
- **pricingService.js**: 定价计算服务，支持Claude和Gemini模型成本核算
- **costInitService.js**: 成本初始化服务，为现有API Key初始化成本跟踪
- **tokenRefreshService.js**: 集中化token刷新服务，处理OAuth token生命周期
- **claudeCodeHeadersService.js**: Claude Code特定头部处理，确保正确的API集成
...
# 数据迁移
npm run migrate:apikey-expiry     # 迁移API Key过期设置
...
- `POST /claude/v1/messages` - Claude标准格式端点（与/api功能相同）
...
### Gemini API端点
- `POST /gemini/v1/models/*/generateContent` - Gemini消息生成
- `POST /openai/gemini/v1/chat/completions` - OpenAI兼容的Gemini端点
...
### 系统端点
- `GET /health` - 健康检查，包含组件状态和版本信息
- `GET /metrics` - 系统指标，包含内存使用和运行时统计
- `GET /web` - Web管理界面
...
- **Gemini 账户**: `gemini_account:{id}` (Gemini API Key和配置)
- **管理员**: `admin_credentials` (全局管理员凭据) + `admin_session:{token}` (会话管理)
- **会话**: `session:{token}` (JWT 会话管理)
- **使用统计**: `usage:daily:{date}:{key}:{model}` (多维度统计)
- **成本跟踪**: `cost:daily:{date}:{key}` (每日成本统计) + `cost:total:{key}` (总成本)
- **系统信息**: `system_info` (系统状态缓存) + `pricing_cache` (定价缓存)
- **速率限制**: `rate_limit:{key}:{window}` (API Key级别速率限制)
- **并发控制**: `concurrency:{key}` (API Key并发请求计数)
