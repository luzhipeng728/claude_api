...
const app = createApp({
    data() {
        return {
            ...
            accountSortBy: 'username', // 默认按用户名数字部分排序
    ...
    methods: {
        ...
        async openEditAccountModal(account) {
            ...
            // 如果是Claude账户，尝试获取完整信息（包含tokens）
            if (account.platform === 'claude') {
                try {
                    const response = await fetch(`/admin/claude-accounts/${account.id}`, {
                        headers: {
                            'Authorization': `Bearer ${this.authToken}`
                        }
                    });
                    ...
                    if (response.ok) {
                        ...
                        if (result.success && result.data) {
                            // 更新tokens信息
                            this.editAccountForm.accessToken = result.data.accessToken || '';
                            this.editAccountForm.refreshToken = result.data.refreshToken || '';
                        }
                    }
                } catch (error) {
        ...
        sortAccounts() {
            ...
            this.accounts.sort((a, b) => {
                switch (this.accountSortBy) {
                    ...
                    case 'username':
                        // 获取用户名
                        const aUsername = (a.proxy && a.proxy.username) || '';
                        const bUsername = (b.proxy && b.proxy.username) || '';
                        
                        // 提取用户名中的数字部分 (例如 "xmfwgiio-18" 中的 18)
                        const aMatch = aUsername.match(/-(\d+)$/);
                        const bMatch = bUsername.match(/-(\d+)$/);
            ...
            try {
                const data = await this.apiRequest('/admin/api-keys', {
                    ...
                    body: JSON.stringify({
                        ...
                        keyType: this.apiKeyForm.keyType || 'cc',
            ...
            try {
                const data = await this.apiRequest('/admin/api-keys/' + this.editApiKeyForm.id, {
                    ...
                    body: JSON.stringify({
                        ...
                        keyType: this.editApiKeyForm.keyType || 'cc',
